...
public:
    ...
    double mag2() const
    { // squared size of vector (slightly faster)
        return x * x + y * y + z * z;
    }
    double mag() const
    { // size of vector
    ...
    { // multiplication of vector by scalar (vec x scalar)
...
{ // multiplication of scalar by vector (scalar x vec)
    return y * scalar;
}
...
//#pragma omp simd private(target_num) aligned(distances2 : 64) // SIMD pragma to parallelize the loop
    ...
    {                                        // For each molecule, build the neighbour list
        ...
        for (size_t j{}; j < target_num; j++)
        {
            ...
            if (k < i)
            { // neighbour list of molecule k has already been created
                ...
                if (std::find(neighbours.begin(), neighbours.end(), i) == neighbours.end()) // molecule i is not in neighbour list of molecule k
                    sys.molecules.neighbours[i].push_back(k);                               // add molecule k to the neighbour list of molecule i
            }
            else
            {
                sys.molecules.neighbours[i].push_back(k); // add molecule k to the neighbour list of molecule i
...
System MakeWater(size_t N_molecules)
{
    ...
    // initialize all water molecules on a sphere and the atoms
